...
echo
while true ; do
	if [ "$(transmission-remote -t 1 -i | grep Percent | cut -d " " -f 3-)" = "Percent Done: 100%" ] ; then
	...
	[ "$(transmission-remote -t 1 -f | grep Yes | cut -d " " -f 3- | cut -d " " -f -1)" = "75%" ] && echo "75%"
	[ "$(transmission-remote -t 1 -f | grep Yes | cut -d " " -f 3- | cut -d " " -f -1)" = "100%" ] && echo "100%"
  sleep 1
done
}
